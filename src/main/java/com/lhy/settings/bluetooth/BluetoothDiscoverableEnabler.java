...
//            Log.d(TAG, "setEnabled(): enabled = " + enable + "timeout = " + timeout);
    ...
    private void updateTimerDisplay(int timeout) {
        if (getDiscoverableTimeout() == DISCOVERABLE_TIMEOUT_NEVER) {
            SpannableStringBuilder builder = new SpannableStringBuilder(this.mContext.getResources().getText(R.string.bluetooth_is_discoverable_always));
            ForegroundColorSpan redSpan = new ForegroundColorSpan(Color.WHITE);
            builder.setSpan(redSpan, 0, this.mContext.getResources().getText(R.string.bluetooth_is_discoverable_always).length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            mDiscoveryPreference.setSummary(builder);
        } else {
            String textTimeout = formatTimeRemaining(timeout);
            SpannableStringBuilder builder = new SpannableStringBuilder(mContext.getString(R.string.bluetooth_is_discoverable,
                    textTimeout));
            ForegroundColorSpan redSpan = new ForegroundColorSpan(Color.WHITE);
    ...
    private void setSummaryNotDiscoverable() {
        if (mNumberOfPairedDevices != 0) {
            SpannableStringBuilder builder = new SpannableStringBuilder(this.mContext.getResources().getText(R.string.bluetooth_only_visible_to_paired_devices));
            ForegroundColorSpan redSpan = new ForegroundColorSpan(Color.WHITE);
            builder.setSpan(redSpan, 0, this.mContext.getResources().getText(R.string.bluetooth_only_visible_to_paired_devices).length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            mDiscoveryPreference.setSummary(builder);
        } else {

            SpannableStringBuilder builder = new SpannableStringBuilder(this.mContext.getResources().getText(R.string.bluetooth_not_visible_to_other_devices));
            ForegroundColorSpan redSpan = new ForegroundColorSpan(Color.WHITE);
            builder.setSpan(redSpan, 0, this.mContext.getResources().getText(R.string.bluetooth_not_visible_to_other_devices).length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            mDiscoveryPreference.setSummary(builder);
        }
    }
